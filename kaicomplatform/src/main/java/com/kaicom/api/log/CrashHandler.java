...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public static String TAG = "KaicomCrash";
    ...
    private boolean handleException(Throwable ex) {
        ...
        try {
            ...
            // crash
            PlatformPreference.getInstance().setCrash(true);
            
            long curTime = new Date().getTime();
            long lastTime = PlatformPreference.getInstance().getLastCrashTime();
            if (curTime - lastTime > 60 * 1000) {
            	ApkUtil.restart();
            }
            PlatformPreference.getInstance().setLastCrashTime(curTime);
            SystemClock.sleep(3000);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void collectDeviceInfo(Context ctx) {
        try {
        	try {
				infos.put("MACHINE_CODE", KaicomApplication.app.getMachineCode());
			} catch (Exception e) {
			}
            ...
            Log.e(TAG, "an error occured when collect domain info", e);
        }
    ...
    public void autoClear() {
    	FileUtil.delete(getGlobalpath(), new FilenameFilter() {
            ...
            public boolean accept(File file, String filename) {
                ...
                int autoClearDay = PlatformPreference.getInstance().getCrashAutoClearDays();
